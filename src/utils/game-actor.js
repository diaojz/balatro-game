/**
 * game-actor.js — v1.11 AI 托管模式的游戏动作执行手
 *
...
export function createGameActor({ refs, actions }) {
   ...
   *
   * pack 阶段兼容处理（v1.11.0 AI 不处理卡包，统一跳过）：
   ...
   * 前置：runPhase === 'reward'
   *
   * 在当前 v1.10.0 实现中，passBlind() 会在 setTimeout 后自动调用 openShop()，
   ...
   *
   * 当前 v1.10.0 中，ante 推进是 closeShop() 自动完成的，
